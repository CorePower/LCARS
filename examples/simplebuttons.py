...
class SimpleTest(LCARS.Main):
	...
	def create(self):
		...
		btn_foo = self.add_control("btn_foo", Button(Rect(10, elbo.b()+spacing, 100, 40), Cap.NONE, "FOO", clr_unavail, self.background, buttontext))
		btn_bar = self.add_control("btn_bar", Button(Rect(10, btn_foo.b()+spacing, 100, 40), Cap.NONE, "BAR", clr_unavail, self.background, buttontext))
		...
		spacer = self.add_control("spacer", CappedBar(pygame.Rect(10, btn_bar.b()+spacing, 100, btn_exit.t()-btn_bar.b()-2*spacing), Cap.NONE, None, clr_elbos, None, None))
